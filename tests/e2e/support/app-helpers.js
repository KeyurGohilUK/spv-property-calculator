...
export async function blockExternalServices(page, { authenticated = true, policyAccepted = true } = {}) {
  await page.addInitScript(() => localStorage.setItem('spv-help-guide-seen', 'true'));
  await page.route(/^https:\/\/(?!127\.0\.0\.1)/, (route) => route.abort());
  await installCloudMock(page, { signedOut: !authenticated, policyAccepted });
}
