...
alter table public.task_comments add column if not exists updated_at timestamptz null;
...
drop function if exists public.insert_task_comment(text,text,text,text,timestamptz);
