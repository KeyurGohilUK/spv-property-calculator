...
test('uploads, replaces and deletes a receipt through the private Worker contract', async ({ page }) => {
  ...
  expect(firstUpload.headers['x-receipt-name']).toBe('first-receipt.pdf');
  ...
  expect(replacement.headers['x-receipt-name']).toBe('replacement-receipt.pdf');
...
test('unauthorized Worker response keeps the receipt safely pending locally', async ({ page }) => {
  ...
  expect(expense.receipt.name).toBe('protected-receipt.pdf');
